import { stevensCourses, stevensPrograms } from "./stevensData.js";
...
export function resolveProgramId(profile) {
    const plan = profile.stevensPlan;
    ...
    if (hay.includes("information systems") || hay.includes("msis"))
        return "msmis-demo";
    if (hay.includes("computer science") || /\bmscs\b/.test(hay))
        return "mscs-demo";
    if (hay.includes("business analytics") ||
...
export function resolveCatalogForProfile(profile) {
    const programId = resolveProgramId(profile);
    const program = programById(programId);
    const planCodes = collectCodesFromPlan(profile.stevensPlan);
    const selected = new Map();
    for (const raw of planCodes) {
        const hit = findCourseFlexible(raw);
        if (hit)
            selected.set(hit.code, hit);
    }
    if (program) {
...
export function buildAdvisorStevensContext(profile) {
    if (!profile) {
        return [
