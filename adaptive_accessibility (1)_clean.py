...
class AdaptiveLearningCV:
    ...
    def generate_pdf_report(self, filename="accessibility_report.pdf"):
        ...
        # Custom Styles
        title_style = ParagraphStyle('TitleStyle', parent=styles['Heading1'], fontSize=24, textColor=colors.hexColor("#0056D2"), spaceAfter=20, alignment=1)
        sub_title_style = ParagraphStyle('SubTitleStyle', parent=styles['Heading2'], fontSize=18, textColor=colors.hexColor("#1E293B"), spaceAfter=12)
        text_style = styles['Normal']
        metric_title_style = ParagraphStyle('MetricTitle', parent=styles['Heading3'], fontSize=14, textColor=colors.hexColor("#0F172A"), spaceBefore=15)

        # Title
        story.append(Paragraph("STEVE Accessibility Intelligence Report", title_style))
        story.append(Paragraph(f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", text_style))
        story.append(Spacer(1, 0.5 * inch))

        # Executive Summary
        story.append(Paragraph("Executive Summary", sub_title_style))
        summary_text = "This report provides an in-depth analysis of the student's learning engagement, stress levels, and accessibility needs detected via computer vision during the study session."
        ...
        # ── 1. Attention & Focus ──
        story.append(Paragraph("1. Attention & Cognitive Focus", metric_title_style))
        story.append(Paragraph(f"<b>Average Score:</b> {metrics.get('attention_efficiency', 0):.1f}%", text_style))
        story.append(Paragraph("<b>Impact:</b> High attention efficiency correlates with better information retention and reduced study time.", text_style))
        story.append(Paragraph("<b>Accessibility Use:</b> Identifying attention drops helps in recommending optimal 'study sprints' and breaks for students with ADHD or cognitive fatigue.", text_style))
        ...
        story.append(Paragraph("2. Stress & Cognitive Load", metric_title_style))
        story.append(Paragraph(f"<b>Average Stress:</b> {metrics.get('stress_management', 0):.1f}% (Health Score)", text_style))
        story.append(Paragraph("<b>Impact:</b> Sustained high stress can lead to burnout and decreased problem-solving capabilities.", text_style))
        story.append(Paragraph("<b>Accessibility Use:</b> Monitoring stress spikes allows the system to trigger 'calm-down' prompts or simplify complex content for students with anxiety or high cognitive load.", text_style))
        ...
        story.append(Paragraph("3. Engagement & Interaction", metric_title_style))
        story.append(Paragraph(f"<b>Peak Engagement:</b> {metrics.get('peak_engagement', 0):.1f}%", text_style))
        story.append(Paragraph("<b>Impact:</b> Active engagement indicates high-quality learning and interest in the subject matter.", text_style))
        story.append(Paragraph("<b>Accessibility Use:</b> Understanding engagement patterns helps personalize the delivery style (e.g., more interactive labs vs. reading) based on the student's unique sensory preferences.", text_style))

        # Recommendations Table
        story.append(Spacer(1, 0.3 * inch))
        story.append(Paragraph("Strategic Recommendations", sub_title_style))
        
        rec_data = [["Category", "Action Item", "Priority"]]
        for rec in report_data.get('recommendations', []):
            rec_data.append([rec['category'], rec['action'], rec['priority'].upper()])
        
        if len(rec_data) == 1:
            rec_data.append(["General", "Continue current study habits", "LOW"])

        table = Table(rec_data, colWidths=[1.5*inch, 3.5*inch, 1*inch])
